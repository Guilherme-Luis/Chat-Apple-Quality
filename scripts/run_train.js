...
function run(cmd, args) {
  ...
  if (r.status !== 0) throw new Error('Training failed');
}

try {
  const python = pythonInVenv();
  run(python, ['train_models.py', '--processed-csv', 'resources/processed_apple_quality.csv', '--artifacts-dir', 'resources/artifacts_models']);
